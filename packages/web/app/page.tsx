...
const CardTooltip = ({ cardName, x, y }: { cardName: string; x: number; y: number }) => {
  ...
  useEffect(() => {
    const fetchCardImage = async () => {
      try {
        ...
        // Query our local database for the card's image_uris
        const response = await fetch('/api/query', {
          ...
          body: JSON.stringify({ 
            query: `SELECT image_uris FROM cards WHERE name = '${cardName.replace(/'/g, "''")}'` 
          }),
        });
        ...
        if (response.ok) {
          ...
          if (result.success && result.data && result.data.length > 0) {
            const imageUris = result.data[0].image_uris;
            if (imageUris) {
              let parsedImageUris;
              try {
                parsedImageUris = typeof imageUris === 'string' ? JSON.parse(imageUris) : imageUris;
  ...
  return (
    ...
    >
      <div className="bg-gray-800 border border-gray-600 rounded-lg p-2 shadow-xl max-w-sm">
        {isLoading && (
